...
local de Laravel Sanctum. La relación usa `registrations.user_id`; no usar
`student_id`, `organizer_id`, rutas de usuarios por ID ni una versión temporal
de estas APIs.
...
Las tablas, relaciones, estados, factories y datos semilla ya existen en el
modelo normalizado. No crear otra tabla de inscripciones.
...
| --- | --- | --- | --- |
| POST | /api/events/{event}/registrations | Usuario autenticado | Crea o reactiva la inscripción propia. |
| DELETE | /api/events/{event}/registrations | Usuario autenticado | Cancela la inscripción propia activa. |
| GET | /api/events/{event}/registrations | Organizer activo responsable | Lista inscritos activos y cupos. |
| GET | /api/me/registrations | Usuario autenticado | Lista mis inscripciones activas. |

POST y DELETE no reciben cuerpo. El navegador no puede elegir otro usuario.
...
- No exigir un rol global para inscribirse; basta una sesión autenticada.
- Una inscripción active existente devuelve 409.
- Contar solo inscripciones active; si alcanzan capacity, devolver 409.
- Si existe una fila cancelled para ese evento y usuario, reactivarla:
...
- Verificar que el usuario autenticado tiene una membresía activa con rol
  `organizer` en la comunidad del evento; de lo contrario 403.
- Devolver únicamente inscripciones active, ordenadas por registered_at.
- Incluir por asistente: id, registered_at, estado y usuario
  (id, first_name, last_name, email).
- Incluir summary con capacity, active_registrations y available_capacity.

### Mis inscripciones

- Verificar únicamente la sesión autenticada.
...
el recurso de eventos existente. En la lista de asistentes, añadir user.
